...
bcrypt.genSalt(saltRounds, (err:any, salt:any) => {
    if (err) {
        ...
        bcrypt.hash(plaintextPassword, salt, (err:any, hash:any) => {
            if (err) {
                // Manejar el error
            } else {
...
const registrar = (req:any, res:any) => {
    ...
    connection.query(sql_validUser, values_validUser, (err:any, results:any) => {
        if (err) {
...
const changePassword = (req:any, res:any) => {
    ...
    connection.query(sql_valid_email, value_valid_email, (err:any, results:any) => {
        if (err) {
            console.error('Error ejecutando la consulta:', err);
            return res.status(500).send('Error en el servidor');
        }
            ...
            if (err) {
                ...
                bcrypt.hash(newPassword, salt, (err:any, hash:any) => {
                    if (err) {
                        ...
                        connection.query(sqlUpdate, value_update_password, (err:any, results:any) => {
                            if (err) {
...
const login = (req:any, res:any) => {
    ...
    connection.query(sql_validUser, values_validUser, (err:any, results:any) => {
        ...
        if (results.length == 0) {
            ...
            bcrypt.compare(password, results[0].clave, (err:any, result:any) => {
                if (err) {
